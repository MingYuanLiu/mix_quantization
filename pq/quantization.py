...
def main():
    ...
    layers = [layer for layer in watcher.layers[1:] if args.block in layer and not 'se' in layer]
    print(' ===> layers: ', layers)

    # data loading code
    train_loader, test_loader = load_dataset(type='imagenet', data_path=args.data_path, batch_size=args.batch_size, nb_workers=args.n_workers)
