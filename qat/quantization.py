# Reference from https://github.com/666DZY666/micronet
# 
...
class MinMaxObserver(ObserverBase):
    def __init__(self, q_level, out_channels):
        ...
        if self.q_level == 'L':
            self.register_buffer('min_val', torch.zeros((1), dtype=torch.float32, device=self.device))
            self.register_buffer('max_val', torch.zeros((1), dtype=torch.float32, device=self.device))
        elif self.q_level == 'C':
            self.register_buffer('min_val', torch.zeros((out_channels, 1, 1, 1), dtype=torch.float32, device=self.device))
            self.register_buffer('max_val', torch.zeros((out_channels, 1, 1, 1), dtype=torch.float32, device=self.device))
        elif self.q_level == 'FC':
            self.register_buffer('min_val', torch.zeros((out_channels, 1), dtype=torch.float32, device=self.device))
            self.register_buffer('max_val', torch.zeros((out_channels, 1), dtype=torch.float32, device=self.device))
...
class Quantizer(nn.Module):
    def __init__(self, bits, observer, activation_weight_flag, qaft=False, union=False):
        ...
        if self.observer.q_level == 'L':
            self.register_buffer('scale', torch.ones((1), dtype=torch.float32, device=self.device))
            self.register_buffer('zero_point', torch.zeros((1), dtype=torch.float32, device=self.device))
        elif self.observer.q_level == 'C':
            self.register_buffer('scale', torch.ones((self.observer.out_channels, 1, 1, 1), dtype=torch.float32, device=self.device))
            self.register_buffer('zero_point', torch.zeros((self.observer.out_channels, 1, 1, 1), dtype=torch.float32, device=self.device))
        elif self.observer.q_level == 'FC':
            self.register_buffer('scale', torch.ones((self.observer.out_channels, 1), dtype=torch.float32, device=self.device))
            self.register_buffer('zero_point', torch.zeros((self.observer.out_channels, 1), dtype=torch.float32, device=self.device))
        self.register_buffer('eps', torch.tensor((torch.finfo(torch.float32).eps), dtype=torch.float32, device=self.device))
...
class SignedQuantizer(Quantizer):
    def __init__(self, *args, **kwargs):
        ...
        if self.activation_weight_flag == 0:   # weight
            self.register_buffer('quant_min_val', torch.tensor((-((1 << (self.bits - 1)) - 1)), dtype=torch.float32, device=self.device))
            self.register_buffer('quant_max_val', torch.tensor(((1 << (self.bits - 1)) - 1), dtype=torch.float32, device=self.device))
        elif self.activation_weight_flag == 1: # activation
            self.register_buffer('quant_min_val', torch.tensor((-(1 << (self.bits - 1))), dtype=torch.float32, device=self.device))
            self.register_buffer('quant_max_val', torch.tensor(((1 << (self.bits - 1)) - 1), dtype=torch.float32, device=self.device))
        else:
...
class UnsignedQuantizer(Quantizer):
    def __init__(self, *args, **kwargs):
        ...
        if self.activation_weight_flag == 0:   # weight
            self.register_buffer('quant_min_val', torch.tensor((0), dtype=torch.float32, device=self.device))
            self.register_buffer('quant_max_val', torch.tensor(((1 << self.bits) - 2), dtype=torch.float32, device=self.device))
        elif self.activation_weight_flag == 1: # activation
            self.register_buffer('quant_min_val', torch.tensor((0), dtype=torch.float32, device=self.device))
            self.register_buffer('quant_max_val', torch.tensor(((1 << self.bits) - 1), dtype=torch.float32, device=self.device))
        else:
            print('activation_weight_flag error')
...
LayerNameStack = []
def judge_include_layer(include_layers):
    global LayerNameStack
    concat_layer_name = '.'.join(LayerNameStack)
    print('concat layer name: ', concat_layer_name)
    print('include layer: ', concat_layer_name in include_layers)
    return concat_layer_name in include_layers
    


def add_quant_op(module, include_layers, a_bits=8, w_bits=8, q_type=0, q_level=0, weight_observer=0,
    ...
    for name, child in module.named_children():
        ...
        if name != 'module':
            LayerNameStack.append(name)
        if isinstance(child, nn.Conv2d) and judge_include_layer(include_layers):
            if bn_fuse:
                conv_name_temp = name
        ...
        elif isinstance(child, nn.Linear) and judge_include_layer(include_layers):
            if child.bias is not None:
        ...
        #    module._modules[name] = quant_concat
            ...
            add_quant_op(child, include_layers, a_bits=a_bits, w_bits=w_bits, q_type=q_type,
                         q_level=q_level, weight_observer=weight_observer,
                         bn_fuse=bn_fuse, bn_fuse_calib=bn_fuse_calib,
                         quant_inference=quant_inference,
                         pretrained_model=pretrained_model,
                         qaft=qaft, ptq=ptq, percentile=percentile)
        if len(LayerNameStack) > 0:
            LayerNameStack.pop()

def prepare(model, include_layers, inplace=False, a_bits=8, w_bits=8, q_type=0, q_level=0,
            weight_observer=0, bn_fuse=False, bn_fuse_calib=False,
            quant_inference=False, pretrained_model=False, qaft=False,
            ptq=False, percentile=0.9999):
    if not inplace:
        model = copy.deepcopy(model)
    add_quant_op(model, include_layers, a_bits=a_bits, w_bits=w_bits, q_type=q_type,
                 q_level=q_level, weight_observer=weight_observer,
                 bn_fuse=bn_fuse, bn_fuse_calib=bn_fuse_calib,
                 quant_inference=quant_inference,
                 pretrained_model=pretrained_model,
