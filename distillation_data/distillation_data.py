...
# tensorboard writer
writer = SummaryWriter(comment = "DistilData")
...
def getRandomData(dataset='imagenet', batch_size=512, for_inception=False, num_workers=32):
    """
...
def getDistilData(teacher_model,
    ...
    for i, gaussian_data in enumerate(dataloader):
        ...
        for it in range(100):
            ...
            writer.add_scalar("Mean loss", mean_loss, it)
            writer.add_scalar("Std loss", std_loss, it)
            # print
            if it % 5 == 0:
                print("[Iter-{}] total_loss: {}, mean_loss: {}, std_loss: {}".format(it, total_loss, mean_loss, std_loss))

        # set targets to 0 for distillation data
        refined_gaussian.append((gaussian_data.detach().clone(), torch.zeros(batch_size, dtype=torch.long)))
